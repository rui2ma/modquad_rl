...
from gym_pybullet_drones.envs.ProgressionAviary_Modquad import ProgressionAviary
...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True, eval=False):
    ...
    if not eval:
        ...
        train_env = make_vec_env(ProgressionAviary,
                                env_kwargs=dict(obs=DEFAULT_OBS, act=DEFAULT_ACT, drone_model=drone_model),
                                n_envs=4,
                                seed=0
                                )
        eval_env = ProgressionAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT, drone_model=drone_model)
        ...
        model = PPO('MlpPolicy',
                    ...
                    tensorboard_log='./results_modquad/tensorboard_logs/',
                    verbose=1)
        ...
        model.learn(total_timesteps=2*int(1e6) if local else int(1e2), # shorter training in GitHub Actions pytest
        ...
        filename=os.path.join(output_folder, "save-03.12.2024_18.54.13")
