...
from gym_pybullet_drones.envs.ProgressionDemoAviary import ProgressionDemoAviary
...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True, eval=False):
    ...
    ############################################################
    ############################################################
    filename=os.path.join(output_folder, "1POINT")
    ...
    test_env = ProgressionDemoAviary(waypoints=np.array([[0,0,1]]),
                                     initial_xyzs=np.array([[0,0,0.5]]),
                                    #  initial_rpys=np.array([[-np.pi/4, 0,0]]),
                                     test_flag=True,
                                     gui=gui,
                                     record=record_video)
