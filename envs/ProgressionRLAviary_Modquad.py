...
from gym_pybullet_drones.envs.ProgressionBaseAviary_Modquad import ProgressionBaseAviary
...
class ProgressionRLAviary(ProgressionBaseAviary):
    ...
    def _preprocessAction(self,
        ...
        rpm = np.zeros((self.NUM_DRONES,self.num_rotors))       # modified by rui
        for k in range(action.shape[0]):
            ...
            if self.ACT_TYPE == ActionType.RPM:
                ...
                rpm[k,:] = np.repeat(self.HOVER_RPM * (1+0.05*target), self.num_rotors)          # modified by rui
    ...
    def _observationSpace(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            #obs_waypoints
            obs_lower_bound = np.hstack([obs_lower_bound, np.array([[lo,lo,lo]*self.num_waypoints for i in range(self.NUM_DRONES)])])
            obs_upper_bound = np.hstack([obs_upper_bound, np.array([[hi,hi,hi]*self.num_waypoints for i in range(self.NUM_DRONES)])])
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            ############################################################
            #### OBS SPACE OF SIZE 17 + 1 waypoint
            #### z, rot, v, ang + waypoint
            ...
            ret = np.hstack((obs_16, obs_waypoints))
            return ret
            ############################################################
        else:
