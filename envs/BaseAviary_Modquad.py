...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.HOVER_RPM = np.sqrt(self.GRAVITY / (8*self.KF))        # modified by rui
        self.MAX_RPM = np.sqrt((self.THRUST2WEIGHT_RATIO*self.GRAVITY) / (8*self.KF))       # modified by rui
        self.MAX_THRUST = (8*self.KF*self.MAX_RPM**2)       # modified by rui
        if self.DRONE_MODEL == DroneModel.CF2X or self.DRONE_MODEL == DroneModel.TBO:
            self.MAX_XY_TORQUE = (2*self.L*self.KF*self.MAX_RPM**2)/np.sqrt(2)
        elif self.DRONE_MODEL == DroneModel.CF2P:
            self.MAX_XY_TORQUE = (self.L*self.KF*self.MAX_RPM**2)
        elif self.DRONE_MODEL == DroneModel.RACE:
            self.MAX_XY_TORQUE = (2*self.L*self.KF*self.MAX_RPM**2)/np.sqrt(2)
        self.MAX_Z_TORQUE = (4*self.KM*self.MAX_RPM**2)
    ...
    def step(self,
        ...
        if self.USE_GUI_RPM:
            for i in range(8):      # modified by rui
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            clipped_action = np.reshape(self._preprocessAction(action), (self.NUM_DRONES, 8))
    ...
    def step_test(self, action):
        ...
        return obs, reward, terminated, truncated, info, self.rpy[0, :]
    ...
    def _getDroneStateVector(self,
        ...
        state = np.hstack([self.pos[nth_drone, :], self.quat[nth_drone, :], self.rpy[nth_drone, :],
                           ...
                           self.rot_mat[nth_drone, :]])
        return state.reshape(33, )
    ...
    def _getDroneImages(self,
        ...
        #### Set 
        #  point, camera view and projection matrices #
    ...
    def _physics(self,
        ...
        if self.DRONE_MODEL == DroneModel.RACE:
            torques = -torques
        z_torque = (-torques[0] + torques[1] - torques[2] + torques[3]-torques[4] + torques[5] - torques[6] + torques[7])       # torque?
        ...
        p.applyExternalTorque(self.DRONE_IDS[nth_drone],        # torque appled to robot base?
                              0,
    ...
    def _parseURDFParameters(self):
        ...
        """
        URDF_TREE = etxml.parse(pkg_resources.resource_filename('gym_pybullet_drones', 'assets/2x1_modquad/'+self.URDF)).getroot()
